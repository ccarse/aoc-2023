...
def primary_score(hand)
  without_jokers = hand.filter { |card| card != 'J' }
  p "without_jokers: #{without_jokers}"
  if without_jokers.length == 0 then
    best = 'A'
  else
    best = without_jokers.tally.sort_by { |card, count| count }.last[0]
  end

  hand = hand.join("").gsub("J", best).chars
...
def secondary_score(hand)
  cards = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J'].reverse
...
inputs = input.map { |line| line.split(" ") }.map { |hand, bid| [hand.chars, bid] }
